...
@section('content')
<form action="{{ route('info.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg p-8 shadow">
    @csrf
    ...
    <input type="text" name="title" class="bg-slate-100 w-full rounded h-14 p-1 ps-3 pe-3 outline-0" required>

    <div class="text-sm text-slate-700 mb-2 mt-6">Konten</div>
    <div id="editor"></div>
    <textarea name="body" id="body" rows="10" class="hidden">tes</textarea>
    ...
    <div class="bg-slate-100 w-full rounded min-h-14 p-3  outline-0 flex flex-wrap gap-2 items-center">
        ...
        <input type="text" class="bg-transparent h-14 outline-0 flex grow" placeholder="Ketik label (pisahkan dengan koma)" oninput="typeLabel(this)">
    </div>
