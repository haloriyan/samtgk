...
@section('title', "Pembayaran Online Melalui E-Channel")
        ...
        <tbody>
            @foreach ($links as $link)
                <tr class="even:bg-gray-100 hover:bg-gray-200">
                    <td class="p-2">
                        <img src="{{ asset('storage/payment_link_images/' . $link->image) }}" alt="{{ $link->name }}" class="w-32">
                    </td>
                    <td class="p-2">
                        <div class="font-bold">{{ $link->name }}</div>
                        <a href="{{ $link->link }}" class="underline text-blue-500 text-sm mt-1" target="_blank">{{ $link->link }}</a>
                    </td>
                    <td class="p-2">
                        <button class="bg-green-500 hover:bg-green-700 text-white p-2 px-3 rounded text-sm" onclick="edit('{{ $link }}')">
                            <i class="bx bx-edit"></i>
                            Edit
                        </button>
                        <button class="bg-red-500 hover:bg-red-700 text-white p-2 px-3 rounded text-sm" onclick="del('{{ $link }}')">
...
<div id="CreateModal" class="fixed top-0 left-0 right-0 bottom-0 z-50 bg-gray-400 bg-opacity-75 hidden row items-center justify-center">
    <div class="bg-white rounded-lg shadow-dark w-6/12 mobile:w-10/12">
        <form action="{{ route('paymentLink.store') }}" class="p-8 flex grow flex-col" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="flex gap-8 items-center mb-6">
                <h2 class="text-xl text-slate-700 flex grow">Tambah Channel</h2>
            ...
            <div class="text-sm text-slate-500 mb-2">Nama Channel :</div>
            <input type="text" name="name" required placeholder="Tokopedia, Gopay, Klik Indomaret, dll" class="bg-slate-100 w-full rounded h-14 p-1 ps-3 pe-3 outline-0">
...
<div id="EditModal" class="fixed top-0 left-0 right-0 bottom-0 z-50 bg-gray-400 bg-opacity-75 hidden row items-center justify-center">
    <div class="bg-white rounded-lg shadow-dark w-6/12 mobile:w-10/12">
        <form action="{{ route('paymentLink.update') }}" class="p-8 flex grow flex-col" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" id="id">
            <div class="flex gap-8 items-center mb-6">
                <h2 class="text-xl text-slate-700 flex grow">Edit Channel</h2>
            ...
            <div class="text-sm text-slate-500 mb-2">Nama Channel :</div>
            <input type="text" name="name" id="name" required placeholder="Tokopedia, Gopay, Klik Indomaret, dll" class="bg-slate-100 w-full rounded h-14 p-1 ps-3 pe-3 outline-0">
...
<div id="DeleteModal" class="fixed top-0 left-0 right-0 bottom-0 z-10 bg-gray-400 bg-opacity-75 hidden row items-center justify-center">
    <div class="bg-white rounded-lg shadow-dark w-5/12 mobile:w-10/12">
        <form action="{{ route('paymentLink.delete') }}" class="p-8 flex grow flex-col" method="POST">
            @csrf
            <input type="hidden" name="id" id="id">
            <div class="flex gap-8 items-center mb-6">
                <h2 class="text-xl text-slate-700 flex grow">Hapus Channel</h2>
            ...
            <div>
                Yakin ingin menghapus channel ini? Data yang sudah terhapus tidak akan muncul kecuali ditambahkan kembali
            </div>
...
<script>
    ...
    const edit = data => {
        ...
        select("#EditModal #filename").innerHTML = data.image;
    }
</script>
@endsection