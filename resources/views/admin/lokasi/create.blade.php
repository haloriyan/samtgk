...
@section('title', "Tambah Lokasi Pelayanan")

@section('content')
<div class="bg-white rounded-lg shadow p-8">
    <form action="{{ route('lokasi.store') }}" class="flex flex-col grow" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="flex gap-8 items-center mb-6">
            <h2 class="text-xl text-slate-700 flex grow">Tambah Lokasi</h2>
            ...
            <div class="flex flex-col w-6/12">
                ...
                <div class="flex flex-col gap-4 mt-2">
                    @foreach ($layanans as $item)
                        <div class="flex items-center gap-4 rounded-lg border p-4 cursor-pointer" onclick="toggleLayanan('{{ $item->id }}', this)">
                            <div class="flex grow text-sm text-slate-700">{{ $item->name }}</div>
                            <div class="h-6 aspect-square rounded-lg border border-slate-500 checkbox flex items-center justify-center"></div>
                        </div>
                    @endforeach
                </div>
                <input type="text" class="h-2 opacity-0" name="layanan" id="layanan" required>
            </div>
        </div>
